...
#a2=read.table("CVS_Spontaneous_sample_info.csv",sep=',',header=T)
#a3=read.table("CVS_Spontaneous_comparisons.csv",sep=',',header=F)

#project = "CVS_Spontaneous"
comps <- as.matrix(a3) 
...
dim(countdata)
#CVS <- countdata[, grep("CVS", colnames(countdata), value = TRUE)]
#CVS_Spontaneous <- countdata[, grep("Spontaneous", colnames(CVS), value = TRUE)]
#dim(CVS_Spontaneous)
#condition <- factor(c("M",	"M",	"M",	"M",	"F",	"F",	"M",	"F",	"M",	"F",	"M",	"F",	"F",	"F",	"M",	"F",	"M",	"F",	"M",	"F",	"M",	"F",	"F",	"M",	"M",	"F",	"M",	"F",	"M",	"F",	"F",	"F",	"F",	"F",	"M",	"M",	"F",	"M",	"M",	"M",	"F",	"F",	"F",	"M",	"F",	"F",	"M",	"M",	"M",	"F",	"M",	"F",	"F",	"F",	"F",	"F",	"M",	"M",	"M",	"F",	"F",	"F",	"F",	"M",	"F",	"F",	"M",	"M",	"M",	"M",	"F",	"F",	"F",	"M",	"F",	"M",	"F",	"M",	"M",	"M",	"M",	"M",	"M",	"M",	"F",	"F",	"M",	"M",	"M",	"F",	"F",	"F",	"F",	"F",	"F",	"M",	"M",	"M",	"M",	"F",	"F",	"M",	"F",	"M",	"F",	"F",	"F",	"M",	"F",	"F",	"M",	"F",	"M",	"F",	"M",	"M",	"M",	"M",	"M",	"F",	"M",	"M",	"F",	"F",	"F",	"M",	"M",	"M",	"M",	"F",	"F",	"M",	"F",	"M",	"F",	"M",	"F",	"M",	"F",	"F",	"M",	"F",	"M",	"M",	"M",	"M",	"F",	"M",	"F",	"F"))
#coldata <- data.frame(row.names=colnames(CVS_Spontaneous), condition)
#dds <- DESeqDataSetFromMatrix(countData=CVS_Spontaneous, colData=coldata, design=~condition)
...
norm=counts(dds,normalized = T)
write.csv(norm, file=paste(project,"_normalized_UMIs.csv",sep=''),row.names=T)
...
for (i in 1:dim(comps)[1]) {
  ...
  #dds_1=DESeq(dds_1,minReplicatesForReplace = Inf)
  ...
  write.csv(eval(parse(text=paste0("resdata", i))), file=paste(out_folder,name,"_DE_miRNAs_all.csv",sep=''),row.names=F)
  ...
  if (n1 < 2000 & n1 >= 200) {
    ...
    write.csv(resSig,file=paste(out_folder,name,"_DE_miRNAs_padj0.05.csv",sep=''),row.names=F)
    sig_DEGs <- "p_adj < 0.05"
  }
  if (n1 >= 2000) {
    if (n2 < 2000) {
      resSig=subset(eval(parse(text=paste0("resdata", i))), padj<0.01)
      write.csv(resSig,file=paste(out_folder,name,"_DE_miRNAs_padj0.01.csv",sep=''),row.names=F)
      sig_DEGs <- "p_adj < 0.01"
    } else {
      resSig=subset(eval(parse(text=paste0("resdata", i))), padj<0.01 & abs(log2FoldChange)>1)
      write.csv(resSig,file=paste(out_folder,name,"_DE_miRNAs_padj0.01_FC2.csv",sep=''),row.names=F)
      sig_DEGs <- "p_adj < 0.01 and |FC| > 2"
    }
  }
  if (n1 < 200 & n1 >= 20) {
    resSig=subset(eval(parse(text=paste0("resdata", i))), padj<0.1)
    write.csv(resSig,file=paste(out_folder,name,"_DE_miRNAs_padj0.1.csv",sep=''),row.names=F)
    sig_DEGs <- "p_adj < 0.1"
  }
  if (n1 < 20) {
