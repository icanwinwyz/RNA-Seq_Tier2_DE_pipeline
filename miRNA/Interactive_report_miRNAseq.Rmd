---
title: "miRNA-seq Differential Expression Analysis Report (`r name`)"
...
### - Data Overview
* We start by reading in the data. There are `r dim(a1)[1]` miRNAs/piRNAs of `r dim(countdata_1)[2]` samples in the project. Samples are labeled as `r colnames(countdata_1)`, representing `r length(levels(condition))` (i.e. `r paste(levels(condition),collapse=", ")`) groups.
* After removing miRNAs/piRNAs with expression equal to 0, we have `r dim(countdata_1)[1]` miRNAs/piRNAs for further analysis.


### - Principal Component Analysis for miRNAseq samples

* PCA plots of RNA-seq data, based on the top 500 miRNAs/piRNAs by variance across all samples were generated. "`r levels(condition)[1]`" and "`r levels(condition)[2]`" groups are represented in red and blue, repsectively.
\
...
* 3D_PCA was generated using the 500 miRNAs/piRNAs that showing the highest variation. `r levels(condition)[1]` and `r levels(condition)[2]` groups are represented in red and blue, repsectively.
\
...
```{r DE_miRNAs/piRNAs_list, echo=FALSE, message=FALSE}
resdata <- eval(parse(text=paste0("resdata", i)))
names(resdata)[1] <- "miRNAs/piRNAs"
names(resSig)[1] <- "miRNAs/piRNAs"
dat.norm_2=as.matrix(norm_1[resSig$`miRNAs/piRNAs`,])
...
```
### - List of significant DE (differentially expressed) miRNAs/piRNAs
* Significance definition
     + Default significance: p_adj < 0.05
     + Optimized significance threshold is defined based on the number of DE miRNAs/piRNAs
     + DE miRNAs/piRNAs cutoff is flexible

Number of DE miRNAs/piRNAs (p_adj threshold)             | Optimized significance threshold
-------------------------------------------------------- | ------------------------------------
...
* `r dim(resSig)[1]` miRNAs/piRNAs were identified as significant DE miRNAs/piRNAs (`r name`; `r sig_DEGs`), where `r dim(subset(resSig, log2FC>=0))[1]` were upregulated and `r dim(subset(resSig, log2FC<0))[1]` were downregulated.
* The table is showing all miRNAs/piRNAs.  Please check CSV file for all details.
...
datatable(format(resdata[, 1:5], digits=2, justify="centre"), caption = htmltools::tags$caption(
    ...
    'Table 1: ', htmltools::em('List of DE miRNAs/piRNAs')))
```

### - Heatmap of DE miRNAs/piRNAs across all samples
* `r dim(dat.norm_2)[1]` miRNAs/piRNAs with significant variances across the `r dim(dat.norm_2)[2]` samples were utilized to make the heatmap.
*  Zoom in to see miRNAs/piRNAs and/or sample names.
\
\
...
#set color range for heatmap for extreme values
#temp <- scale(t(dat.norm_2))
#temp[temp>2] <- 2
#temp[temp< -2] <- -2
#heatmaply(t(temp),colors=colorRampPalette(c("blue","black","yellow"))(256),k_col=2,Rowv=rowv,dendrogram='row',Colv=colv, scale="none",cexRow =0.5,trace="none",density.info="none",keysize=.8,key.xlab="")

#heatmaply(dat.norm_2,scale_fill_gradient_fun = ggplot2::scale_fill_gradient2(low = "blue", high = "red", midpoint = 0, limits = c(-2, 2)),k_col=2,labRow = NULL,Rowv=rowv,dendrogram='row',Colv=colv, scale="row",cexRow =0.5,trace="none",density.info="none",keysize=.8,key.xlab="")


#export heatmap as PDF (did not show row names)
...
#colors = c(seq(-2,-0.5,by=0.03),seq(-0.47,0.49,by=0.03),seq(0.52,2,by=0.03))
#my_palette <- colorRampPalette(c("blue", "black", "yellow"))(length(colors)-1)
#did not show rownames (miRNA/piRNA)
...
#show rownames (miRNA/piRNA)
#heatmap.2(dat.norm_2,col=my_palette,dendrogram='row',Rowv=rowv,Colv=colnames(dat.norm_2),scale="row",cexRow =0.5,trace="none",density.info="none",keysize=.8,key.xlab="",margins=c(15,14),breaks=colors)
...
### - Volcano Plot
* Volcano plot reporting adjusted p-value (−log10(p), y axis) as a function of log2 (fold change) between the groups (log2FC, x axis).
* Fold change(`r name`) of the expression level for a given miRNAs/piRNAs was defined as the read counts of the miRNAs/piRNAs in the `r comps[i,][2]` sample divided by the read counts in the `r comps[i,][3]` sample.
* miRNAs/piRNAs that are identified as significant DE miRNAs/piRNAs with `r sig_DEGs` are colored in red.
\
\
\
...
if (n1 >= 2000) {
    ...
    resdata_1$Colour[ resdata_1$p_adj >= 0.01]="Non-significant"
      ...
      resdata_1$Colour[(abs(resdata_1$log2FC) >= 1) & (resdata_1$p_adj < 0.01)]="Significant(padj<0.05,FC>2)"
...
p <- plot_ly(resdata_1, x = ~log2FC, y = ~negative_Log10_padj, type = 'scatter',
        mode = "markers", color = ~Colour, colors = c("black", "red"), text = ~paste('miRNAs/piRNAs: ', `miRNAs/piRNAs`))%>%
...
resdata_1$label<-""  #####this is to save the label of your interested miRNAs/piRNAs
...
* miRNAs/piRNAs that are identified as significant DE miRNAs/piRNAs with `r sig_DEGs` are colored in red (significant upregulated miRNAs/piRNAs) and in blue (significant downregulated miRNAs/piRNAs).
\
...
plot <- ggplot(data=resdata_1, aes(x=log2Mean, y=log2FC, colour=Colour)) + geom_point(aes(text = paste("miRNAs/piRNAs: ", resdata_1$Gene)), size = 1) + scale_color_manual( values=c("dark gray", "blue", "red")) + labs(x = "Log2 Mean_Expression", y = "Log2 Fold_Change", title = name)
gg <- ggplotly(plot)
gg
